...
Z = unumpy.log(N)
Z_besser = Z[0:12]
plt.errorbar(t, noms(Z), xerr = 0, yerr = stds(Z), fmt = "rx", label = r"Messwerte")

# Ausgleichgerade durch alle und durch die ersten 13 Messwerte legen
...
t_lin_besser = np.linspace(30, 410)

plt.plot(t_lin, f(t_lin, *params), "b", label = r"Lin. Regression 1")
plt.plot(t_lin_besser, f(t_lin_besser, *params_besser), "g", label = r"Lin. Regression 2")
...
plt.errorbar(t_Rhodium, noms(Z_Rhodium), xerr = 0, yerr = stds(Z_Rhodium), fmt = "rx", label = r"Messwerte")
...
Z_kurz = np.log(N_Rhodium_korr[0:12])
...
plt.plot(t_lin_Rhodium, f(t_lin_Rhodium, *params_lang), "b", label = r"Lin. Regression langsamer Zerfall")
#plt.plot(t_lin_Rhodium, f(t_lin_Rhodium, *params_kurz), "g", label = r"Schneller Zerfall Lin. Regression")

plt.plot(t_lin_Rhodium, g(t_lin_Rhodium, N_0_Faktor_lang, lam_lang, N_0_Faktor_kurz, lam_kurz), "g", label = r"Theoretische Kurve")
plt.xlabel(r"$\Delta t$ [s]")
plt.ylabel(r"$ln{N}$")
plt.ylim(0)
plt.legend(loc = "best")
...
t_lin_Rhodium_kurz = np.linspace(0, 200)

plt.plot(t_kurz, noms(Z_kurz), "rx", label = r"Messwerte")
plt.plot(t_lin_Rhodium_kurz, f(t_lin_Rhodium_kurz, *params_kurz), "b", label = r"Lin. Regression schneller Zerfall")
